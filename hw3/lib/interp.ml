  ...
  fun env e ->
    begin match e with
      ...
      | Lst [Sym "let"; Lst l; body] ->
          ...
          let old_env = env
          in
          let env = (List.fold_left (fun env b -> Symtab.add (fst b) (interp_expr old_env (snd b)) env) env bindings)
          in
          interp_expr env body

      | Lst (Sym "case" :: expr :: cases) ->
        (match (interp_expr env expr) with
        | Num n -> (
          let cases = get_cases cases
          in
          let sexp = List.assoc_opt n cases
          in 
          (match sexp with
          | Some s -> interp_expr env s
          | None -> interp_expr env (snd (List.nth cases (List.length cases - 1))))
        )
        | _ -> raise (Stuck expr)) 
      ...
      (* | Lst [Sym "and"; arg1; arg2] -> 
          if interp_expr env arg1 = Bool false || interp_expr env arg2 = Bool false then
            Bool false
          else 
            Bool true
      ...
      | Lst [Sym "and"; arg1; arg2] -> 
          if interp_expr env arg1 = Bool false then
            Bool false
          else 
