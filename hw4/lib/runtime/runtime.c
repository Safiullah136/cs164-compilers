...
#define list_mask   0b11111111
#define list_tag   0b11111111
#define list_shift 8 
...
uint64_t print_value(uint64_t value) {
  if ((value & num_mask) == num_tag) {
    ...
    print_value(v2);
    printf(")");
  } else if ((value & list_mask) == list_tag) {
    printf("()");
  } else if ((value & heap_mask) == vector_tag) {
    printf("[");
    uint64_t len = *((uint64_t *) (value - vector_tag)) >> num_shift;
    for(uint64_t i = 1; i < len; i++) {
      uint64_t v = *((uint64_t *) (value - vector_tag + i * 8));
      print_value(v);
      printf(" ");
    }
    print_value(*((uint64_t *) (value - vector_tag + len * 8)));
    printf("]");
  } else {
