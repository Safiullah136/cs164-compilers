...
let rec display_value : value -> string =
  fun v ->
    begin match v with
      ...
      | Vector v -> 
        sprintf "[" ^ String.concat " " (List.map display_value (Array.to_list v)) ^ "]"
    end

let rec is_list : value -> value option = 
    fun v -> (match v with
        | Pair(_, r) -> is_list r
        | EmptyList -> Some(Bool true)
        | _ -> Some(Bool false))
...
let interp_unary_primitive : string -> value -> value option =
  fun prim arg ->
    begin match (prim, arg) with
      ...
      | ("vector-length", Vector v) -> Some (Num (Array.length v) )
...
let interp_binary_primitive : string -> value -> value -> value option =
  fun prim arg1 arg2 ->
    begin match (prim, arg1, arg2) with
      ...
      | ("vector", Num n, e) when n >= 0 ->
          Some (Vector (Array.make n e))

      | ("vector-get", Vector v, Num n) when n >= 0 && n < (Array.length v) ->
          Some (Array.get v n)
 ...
 string -> value -> value -> value -> value option =
  fun prim arg1 arg2 arg3 ->
    begin match (prim, arg1, arg2, arg3) with
      | ("vector-set", Vector v, Num n, e) when n >= 0 && n < (Array.length v) ->
        Array.set v n e;
        Some (Vector v)
...
let rec interp_expr : environment -> s_exp -> value =
  fun env e ->
    begin match e with
      ...
      | Lst l when List.length l == 0 -> EmptyList
