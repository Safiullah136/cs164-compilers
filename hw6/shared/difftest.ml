...
module Make(I : Infra.T) : T = struct
  ...
  let ast =
    try
      Ok (if is_mlb then I.parse Infra.Mlb program else I.parse Infra.Lisp program)
  ...
  and compiler =
    wipe_tmp () ;
    try_map I.compile ast
    |> function
    | Ok instrs ->
        Assemble.eval_input "test_output" I.runtime_object_file name [] instrs input
    | Error err ->
