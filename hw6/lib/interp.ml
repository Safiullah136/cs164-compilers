...
let rec get_list : value -> s_exp -> value list = 
  fun l e -> (match l with
    | Pair(l, r) -> [l] @ (get_list r e)
    | Nil -> [] 
    | _ -> raise (Stuck e)
  )

let rec get_lisp_list : value list -> value =
  fun l -> (match l with
  | [] -> Nil
  | l :: r -> Pair(l, (get_lisp_list r))  
  )  
  ...
  fun defns env e ->
    begin match e with
      ...
      | Lst [Sym "apply"; Sym f; l] when is_defn defns f -> 
        ...
        let args = get_list (interp_expr defns env l) e in
        if (List.length args = List.length defn.args) || (Option.is_some defn.rest && List.length args >= List.length defn.args) 
        then
          let args, rest = args |> List.partition_at (List.length defn.args) in
          let fenv =
            args
          ...
          if (Option.is_some defn.rest) then
            interp_expr defns (Symtab.add (Option.get defn.rest) (get_lisp_list rest) fenv) defn.body
          else 
      ...
      | Lst (Sym f :: args) when is_defn defns f ->
          ...
          if (List.length args = List.length defn.args) || (Option.is_some defn.rest && List.length args >= List.length defn.args) 
          then
            let args, rest = args |> List.map (interp_expr defns env)
            |> List.partition_at (List.length defn.args) in
            let fenv = args 
            |> List.combine defn.args
            |> Symtab.of_list
            in
