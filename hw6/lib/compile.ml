...
let rec make_list : int -> int -> directive list =
  fun index args_num -> 
    ...
    (* R9 is used for stack index, R10 for counter and R11 for length of rest list *)
    ; Mov (Reg R11, stack_address (-8))
    ; Sub (Reg R11, Imm args_num)
    ; Cmp (Reg R11, Imm 0)
    ; Jne else_label

      ; Mov (stack_address index, operand_of_nil)
      ; Jmp continue_label

    ; Label else_label
    ; Mov (Reg R9, Imm index)  
    ; Mov (Reg R10, Imm 0)

    ; Label loop_label
    ; Mov (Reg R8, MemOffset(Reg Rsp, Reg R9))
    ; Mov (MemOffset (Reg Rdi, Imm 0), Reg R8)
    ; Add (Reg Rdi, Imm 16)
    ; Mov (Reg R8, Reg Rdi)
    ...
    ; Mov (stack_address index, Reg Rax)
    ; Mov (MemOffset(Reg Rdi, Imm (-8)), operand_of_nil)
 ...
 defn list -> symtab -> int -> s_exp -> directive list =
  fun defns tab stack_index e ->
    begin match e with
      ...
      | Lst [Sym "apply"; Sym f; l] when is_defn defns f -> 
          let continue_label = gensym "continue" in
          let loop_label = gensym "loop" in
          let stack_base = align_stack_index (stack_index + 8) in
          compile_expr defns tab stack_index l @
          [ Mov (Reg R9, Imm (stack_base - 24)) 
          ; Mov (Reg R10, Imm 0)
          ; Label loop_label
          ; Cmp (Reg Rax, Imm nil_tag)
          ; Je continue_label
          ] 
          @ ensure_pair (Reg Rax) e @
          [ Sub (Reg Rax, Imm pair_tag)
          ; Mov (Reg R8, MemOffset(Reg Rax, Imm 0))
          ; Mov (MemOffset(Reg Rsp, Reg R9), Reg R8)
          ; Mov (Reg Rax, MemOffset(Reg Rax, Imm 8))
          ; Add (Reg R10, Imm 1)
          ; Add (Reg R9, Imm (-8))
          ; Jmp loop_label
          ; Label continue_label
          ; Mov (stack_address (stack_base - 16), Reg R10)
      ...
      | Lst (Sym f :: args) when is_defn defns f ->
          ...
          if (List.length args = List.length defn.args) || (Option.is_some defn.rest && List.length args >= List.length defn.args) 
          then
            let compiled_args =
              args
                |> List.mapi
...
let compile_defn : defn list -> defn -> directive list =
  fun defns defn ->
      let msg_label = gensym "emsg" in
      let continue_label = gensym "continue" in
      let args = if Option.is_some defn.rest then (defn.args @ [(Option.get defn.rest)]) else defn.args in  
      let rest_offset = (List.length defn.args + 2) * -8 in
      let ftab =
        args
      ...
      in
        ...
        ; Cmp (stack_address (-8), Imm (List.length defn.args)) 
        ; if Option.is_some defn.rest then (Jnl continue_label) else (Je continue_label)
        ; LeaLabel (Reg Rdi, msg_label)
        ; Jmp "lisp_error"
        ; Label msg_label
        ; DqString defn.name
        ...
        @ (if Option.is_some defn.rest 
          then (make_list (rest_offset) (List.length defn.args)) 
          else [])
        @ compile_expr defns ftab (rest_offset - 8) defn.body
        @ [Ret]
