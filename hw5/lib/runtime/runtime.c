...
uint64_t open_in(char *filename) {
  return (open_for_reading(filename) << channel_shift) | in_channel_tag;
}

uint64_t open_out(char *filename) {
  return (open_for_writing(filename) << channel_shift) | out_channel_tag;
}
...
char * input_from_file(int fd, uint64_t heap, int n) {
  char *buf = (char*) (heap + (8 - heap % 8));
  read_all(fd, buf, n);
  return buf;
}

void output_to_file(int fd, char * heap) {
  write_all(fd, heap);
}